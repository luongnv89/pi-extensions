...
- **pi-fusion 0.1.0**: Live model visibility during execution. While a delegation runs, the working row names the executing model with a ticking elapsed time (`Sidekick openai-codex/gpt-5.6-luna · delegation 2/25 · 12s`) and the footer switches to `fusion ▸ <model> <n>s`; idle it shows `fusion sk:<model> <used>/<max> ~$<saved>`, prefixed with `↑<model>` whenever routing has escalated the main agent. `/fusion status` gained a `running now:` line naming either the sidekick or the main agent, and the live main model is tracked through `model_select` so it follows `/model` and Ctrl+P as well as routing. The ticker is unref'd and cleared from a `finally` and on `session_shutdown`.
- **pi-fusion 0.1.0**: Benchmark harness under `bench/` — 4 graded tasks × 2 arms (plain pi vs pi-fusion) on fresh fixture copies, with pre-registered graders, provider-recorded cost accounting, and both arms isolated via `--no-extensions --no-skills --no-context-files`. Measured on `gpt-5.6-sol` main / `gpt-5.6-luna` sidekick: −64.3% on a verification task at equal quality, parity on a trivial lookup, 100% score on both arms throughout. See `bench/results/SUMMARY.md`.
- **pi-fusion 0.1.0**: New extension — a two-agent harness modelled on Devin Fusion. Adds a `delegate` tool backed by a persistent nested Pi agent session on a cheaper sidekick model, with its own tools and its own context, reused across delegations so follow-ups answer from what the sidekick already learned instead of re-shipping context uncached the way an advisor-style tool does. The main agent's system prompt is extended to delegate and monitor while keeping the plan, the interpretation of ambiguity, and the final review; the sidekick is instructed to flag ambiguity rather than guess. The nested session is created with `noExtensions`/`noSkills`/`noContextFiles` so it cannot load pi-fusion recursively and does not pay for the skill roster. Sidekick tools default to read-only (`read, grep, find, ls`) because the nested session has no approval prompts; `/fusion tools coding` is an explicit opt-in for `edit`, `write`, and `bash`. Optional `/fusion routing on` re-evaluates model assignment at compaction boundaries, where the cache miss is already paid: two consecutive failed delegations upgrade the sidekick, continued failure escalates the main agent via `pi.setModel()`, and three clean delegations return it to its baseline. Each delegation reports tokens, actual cost, and an estimate of the same tokens priced at the main model's rates.
...
- **statusline-pi**: Typing no longer lags while the statusline refreshes. `git` and `gh pr view` ran with `execFileSync` every 5 seconds (and `gh` every 5 seconds on branches without a PR), plus once per tool result, blocking Pi's event loop for about 0.1–0.8 s each time in a large repository. They now run asynchronously, one refresh at a time, driven by prompt submit, tool results (coalesced), agent end, `!` commands and branch change, with a 60-second fallback for changes made outside Pi. The PR lookup runs on branch change, then at most every 2 minutes without a PR and every 10 minutes with one. CPU/memory sampling moves to a 10-second in-process timer.
